...
\noindent When the benefits and requirements are identified and the process is approved by a company, it has to be adopted in practice. This requires an answer to questions what, how, who and when. This question is answered by applying Fagerholm et al. model \cite{fagerholm2014building} to two different teams and software products within the case company. \newline
...
In the case company, both of the products are most of the time installed to customer environments. 
...
multiple customer environments with different configurations per customer
...
Specific customer environment, connection might require VPN
-vpn user limit might be full
...
%Problems in current delivery

%Pitkälti käsin. Vaikka se olisi pitkälti automatisoitavissa, sitä ei ole tehty. Säästäisi paljon aikaa, toisi varmuutta. Kun kaikki tehdään käsin, sattuu toisinaan virheitä eikä ole varmuutta tehtiinkö kaikki kuten ennen ja toimiiko kuten pitää. Jos tehtäisiin automaattisesti, olisi tulos konsistentti. Usein joutuu ottamaan etäyhteyksiä remote desktopilla, ja näissä on rajoituksia. VPN:n läpi otettu yhteys katkoo muita yhteyksiä mitä käyttää silloin koneella. Parempi jos tarjottaisiin itse asiakkaalle CDM ympäristöineen.	Vaatii dokumentointia. Kaikkia asioita ei ole dokumentoitu hyvin, prosessi hajautunut. Ei ole tietoa kaikista deploymentin prosesin osista. 	käytännössä mitä tahansa mitä sovitaan, tulee aina ihan helvetin kiire. jos sovimme tuotantoonsiirron 19.9, olemme käytännössä aina enemmän tai vähemmän kusessa sitä edeltävänä päivänä. aina aloitetaan nollapisteestä mitä pitää tehdä tuotantoonsiirron yhteydessä. keksitään aina prosessi uudestaan. automatisointi on hankalaa, koska jos meidän aina pitäisi tehdä täsmälleen samat asiat, tottakai voisimme sitten automatisoida sen jollain tavalla. käytännössä jokainen tuotantoonpäivitys on vähän erilainen kuin edellinen. sis. sovelluksen päivityksen, tuotantodataa saattaa pitää korjata tai ladata lisää.	Adding changes to the database, and doing the testing every time you do the deployment.	arpomista joka kertaa. et voi olla aina varma mitä viet sinne, pitää ehkä purkaa warri ja katsoa mitä sinne on menossa. 	Haasteita mitä ei ole auennut. Releasen rakentamiseen voi mennä päivä työaikaa, kuulostaa oudolta. Versionvaihdon yhteydessä tehtävät tietokantakorjaukset - skriptit ei välttämättä saatavilla ja niitä saattaa jäädä ajamatta. Tässä epätarkkuus. Testaus on haaste, sillä ei ole testisettejä jotka ovat tarpeeksi luotettavia. Asiakkaan kanssa ei haasteita, paitsi jos tuotantoonsiirto ei onnistu.
