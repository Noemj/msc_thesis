...
\begin{center}
    ...
    General & Downtime is more critical for certain customers \\ \hline
    General & Acceptance testing is performed by both the company and the customers \\ \hline
    Configuration & Software is often integrated to multiple third party applications \\ \hline
    Configuration & Software is often accompanied by multiple external components \\ \hline
    Configuration & There exists multiple different configurations due to having multiple customers with different specifications \\ \hline
    Connection & Transferring the software product to diverse customer-owned environments requires different deployment configurations \\ 
...
%Downtime
Downtime of the case company's products can be fatal. With Dialog, downtime causes customers being unable to perform their job. Arbitrary downtime can also interrupt ongoing customer tasks, possibly losing critical data in progress. With CDM, downtime can cause other integrated systems to shut down due to too many failing requests. Currently the deployment time for both projects is negotiated with the customer to prevent either cases. In the case of CDM, downtime must therefore be eliminated by utilizing parallel environments during deployment. In the case of Dialog, the downtime either has to be eliminated or informed to the customer beforehand. 
...
%Automate testing
Along with unit and integration tests, a critical step of the deployment process is the manual acceptance testing phase. The acceptance testing is performed in varying ways. For Dialog, some customers require to perform manual acceptance testing in an UAT environment before the deployment to production can be made. Other customers trust the developers to perform the acceptance testing, and the deployment to production can be negotiated without it necessarily requiring to pass the UAT phase. The technical implementation therefore should make it possible to continuously deploy versions to the UAT environment, and by the push of a button to the production environment.
...
\begin{center}
    ...
    \hline
    \textbf{Problem area} & \textbf{Specific problem} \\ \hline
    Environmental & UAT environment is a requisite for production release \\ \hline
	Procedural & Triggering the compilation and deployment of large, modular projects \\ \hline
	Procedural & The software has to be deployed to multiple customer environments \\ \hline
	Procedural & Versioning is affected by having different customer profiles of the product \\ \hline
	Roles & Responsibility moves towards developers \\
    \hline
    \end{tabular}
\end{center}
...
\begin{center}
    ...
    \hline
    \textbf{Problem area} & \textbf{Specific problem} \\ \hline
    Training & Customers are trained to use a certain UI \\ \hline
    New release & Customers are reluctant towards new versions \\ \hline
	New release & Production deployment schedule has to be negotiated with the customer \\ \hline
	New release & Ongoing critical tasks by users cannot be interrupted with downtime \\ \hline
	New release & Pilot customer is required for developing the process \\ \hline
	New release & Changelogs are especially important, since customers depend on the software \\ 
    \hline
    \end{tabular}
\end{center}
...
The challenges regarding continuous experimentation will be analyzed in four areas: technical, organizational, customer and A/B-testing. A/B testing was chosen as an examined subject of its own, since it is a commonly known experiment, and the difference between the case company's products mostly affects A/B testing.  
...
\begin{center}
    ...
    \hline
    \textbf{Problem area} & \textbf{Specific problem} \\ \hline
    CDM & Not having a UI limits the scope of experiments \\ \hline
	CDM & The system is only usable via API calls, which limits the scope of experiments \\ \hline
	General & Measurable metrics for the software products are not extensively identified  \\ \hline
	Data collection & Data has to be collected in customer environments \\ 
    \hline
    \end{tabular}
\end{center}
...
The business model for the products limits the wanted improved metrics. Since the purpose of the software components isn't to generate revenue per se, knowledge of how the customers benefit from the product is required before the metrics can be chosen. The products also have custom solutions for different customers, which means a measurable property might only apply to a single customer. This makes it somewhat harder to pick the correct measurement variables. 
\begin{aquote}{Dialog developer}
...
\end{aquote}
A possibility is to tone the product in cooperation with the customer, using their internal measures as metrics.
\begin{aquote}{Dialog developer}
...
\end{aquote}
While it is common for B2C applications, too, to fluctuate between different Overall Evaluation Criterias (OEC), in the B2B the possibility to co-operate with the customer should be considered. Eventually, the OEC has to be well thought. For example pure profit is not a good OEC, since raising price can increase a short-term profit but have a negative impact in the long run \cite{kohavi2013online}. However, a clear difference exists between metrics that are not measurable and not-economical to measure \cite{kohavi2013online}. The former is a bad way to run a business, since without being able to articulate what the purpose of the experiment is, it is impossible by the organization to determine whether the experiment is succesful \cite{kohavi2013online}. 

In the case company the developers are not actively participating in customer interaction, and are therefore not as aware of the customers challenges are the team leaders and product owners. 
...
\begin{center}
    ...
    \hline
    \textbf{Problem area} & \textbf{Specific problem} \\ \hline
    Resources & Requirements by customers bring in revenue, and are prioritized over own product development \\ \hline
    Resources & Competence in experimentation is not high, and requires education \\ \hline
    Culture & Transitioning towards experimentation culture affects other teams, including sales \\ \hline
    Organizational divergency & \\ 
    \hline
    \end{tabular}
\end{center}
...
\begin{center}
    ...
    \hline
    \textbf{Problem area} & \textbf{Specific problem} \\ \hline
    Origin of features & A large part of developed features are requirements from customers \\ \hline
	Downtime & Customers perform routine tasks that cannot be interrupted \\ \hline
	General & Customers have to be informed for major changes \\ \hline
	General & Technical personnel aren't involved in customer interaction \\ \hline
	Data collection & The software has to undergo testing in the UAT before production release \\ \hline
	Data collection & The end-users might be customers' customers, complicating feedback collection \\ \hline
	Data collection & Proper legal agreements need to be made with the customers to collect data \\ 	
    \end{tabular}
\end{center}

%Customers have actual needs
In the case company, features have been primarily created for actual customer requirements. This somewhat limits the direction of product development. In the case company, almost 100\% of the completed features have been determined by the customer. This is what a product owner has to say:
...
\begin{center}
    ...
    \hline
    \textbf{Problem area} & \textbf{Specific problem} \\ \hline
    User amount & Low user volume limits the scopes of experiments \\ \hline
	CDM & No users, only integrated systems \\ \hline
	%CDM & No user interface to tune \\ \hline
    %Environment & UAT \\ \hline
    General & Measurable metrics for A/B testing are not extensively identified \\ \hline
	Dialog & Users have been educated to use certain features \\ 
    \hline
    \end{tabular}
\end{center}
...
%TODO: jatka tästä
Most members of the CDM team proposed performance as the only improvable metric. 
...
%TODO: jatka tästä dialogin metriikkoja. Mitä haastateltavat antoivat metriikoiksi?
Members of the Dialog team proposed ..
...
However, without comprehensive automated testing and with the broad scale of features in both software products, in the deployment phase various features are occasionally forgotten and not tested. The features can remain broken or contain bugs when the users start using the new version. Even though these features get fixed relatively fast, this increases the customer reluctancy towards new versions.
...
TODO: list benefits of continuous delivery here, attach them to problems
...
One of the key factors in applying the concept is finding a pro-active lead customer that is willing to explore the new engagement model. While certain types of experiments can be performed without the consent of the customer, experiments can degrade the user experience should be informed to the customer. As the customers acknowledge that the product they have bought is being developed to better suit their needs, approval of the experiments is more likely. 
...
\begin{enumerate}
  \item 
  \item Define an OECs for both projects
