...
%Human error factor
The manual build process involves human error factor, which in the case company is the root cause for majority bugs in new versions. One example of such an error is using the wrong commands to compile and package the project, which causes wrong binaries getting under the new versions. An important factor increasing the rate of human error factor is partially missing documentation and the complexity of the build process. Some steps of the deployment process are documented, but there are still parts that are memorized by developers. According to a developer:
\begin{aquote}{CDM developer}
