...
The case company's products represent two totally different styles of software products. While Dialog is used by users via a UI, CDM is used by other systems via a REST API. 

As CDM is a software component which is primarily used by other systems via API's and certain calls, tracking user behavior doesn't provide much input. CDM also isn't used by an user interface, which limits the possibilities for A/B testing. 

The difference between the products limits the possible scope of experiments. While it is possible to experiment with a software component that has a UI and human users by employing A/B testing, 
-Feature alpha
...
Measurable metrics:
-The products have custom solutions for different customers, and the measurable properties might be only on one customer. Picking the measurement variable therefore is hard.

%CDM is a software component with no user interface, which limits possibilities for A/B testing. 
...
%"We should always think what the customers status is now, what kind of features the customer is using and what is the system that CDM replaces. Against these things we can develop the experimental features. In the old system the cust"
...
%"Asiakkaalta pitäisi saada tähän lupa. Käyttäjät tekevät rutiininomaisia toiminnallisuuksia pitkälti - pitäisi olla vakaa, jotta käyttäjät tietävät mitä missäkin on milloinkin. 
...
%Ei tavoitella klikkejä tai ostotapahtumien määrää, joten tähän ei nähdä mitään syytä. Jos haluttaisiin parantaa verkkokaupan kaupankäyntiä, voitaisiin mitata myyntiä. Dialogissa ehkä enemmän saataisiin siitä irti mitä mieltä asiakkaat ovat. Se voisi olla mittari tuleeko työtehtävät tehtyä nopeammin, mutta se on jo maikea mitata. 
...
%käyttäjiä on liian vähän. jos olisi enemmän aikaa, voisi kokeilla parempia leiskoja. ei tietoa miten palaute otettaisiin vastaan. pitäisi mitata esimerkiksi kuinka nopeasti asiakas löytää jonkun actionin tekemisen. 	Jos tehdään nopeita kokeiluja, ei usein enää tiedetä mitä on asiakkaalla ajossa. Jos tulee ongelmia, ei välttämättä osata heti sanoa mistä ne johtuvat. Yleensä asiakkaan testiympäristö ei ole kauhean kovassa käytössä, ja jos sinne deployataan jotain uutta saattaa kestää kauan että asiakas reagoi siihen. Omassa testipalvelimessa kokeilulla saadan nopeammin palautetta. Asiakkaalle täytyy ilmoittaa, jos tehdään suurempi muutos. Asiakkaan pitää tietää miten järjestelmä toimii, ja jos tehdään yhtään mitään muutoksia, voi se heti aiheuttaa asiakkaalle ongelmia. Jos yhtäkkiä ilmestyy uusi kenttä, asiaks ei välttämättä tiedä mitä sillä tehdään. Muutosten teko ilman ennaltasovittua aikataulua voi olla hankalaa. Teoriassa voi olla asiakas, joka ei halua että me keräämme heiltä tietoa."


%HOW is feedback collected?
%Product ownerit ovat jatkuvassa yhteydessä asiakkaisiin, ja asiakkaat voivat lähettää kommentteja/toiveita/pyyntöjä. Statuspalavereissa asiakkaiden kanssa saadaan palautetta. 	Sähköpostitse saadaan palautetta.	ei ole yhtä toimivaa mallia. meidän projekteissa palautetta kerätään kokoajan, pidämme feedback looppia niin lyhyenä. aina saadaan palautetta kun ollaan tehty jotain. projektitasolla lopussa pidetään erillinen retro asiakkaan kanssa, missä käydään projekti läpi. asiakaspalaute on jatkuvaa, riippuu siitä miten asiakkaan kanssa on sovittu että juttuja käydää käpi. lähinnä sitä saadaan eikä kerätä. jos teemme jonkun featuren ja laitamme sen UAT:hen, emme kerää palautetta vaan asiakas sanoo että joku on mennyt väärin tai kaikki toimii. tästä saadaan palautetta. projektihommissa retrossa kerätään palautetta. 	I dont know. After the project maybe. With Sanoma they had a meeting for feedback after the project.	retroissa projektien lopuksi, sanomissa oli tällainen.	ei hirveästi mitenkään. asiakas meilaa ja soittaa jos joku ei toimi. tikettien kautta, voidaan mitata missä ajassa pystytty ratkomaan. Dialogiin mietitty NPS:ää. Joskus ollut tyytyväisyyskyselyitä ja retroja asiakkaiden kanssa.		En osaa sanoa. Tarkoitus olisi jatkossa tehdä vakiomuotoinen kysely, joka on tietyn tuotteen tarpeisiin kehitetty. 	konsultit juttelevat asiakkaiden kanssa ja kertovat devaajille. on tullut hyviä ja huonoja toiveita. 	Asiakkaan kanssa keskustelevat ihmiset kyselevät asiakkailta miten hommat toimii. Ticketöintijärjestelmään raportoidaan ongelmia. Yleensä tieto tulee Trelloon production bugina, devaajalla ei välttämättä tietoa mistä tullut.	Tällä hetkellä face-to-face. Se mitä saadaan heiltä. Sitä ei kerätä vaan saadaan. Palautetta saadaan paljon koska ollaan paljon asiakaskontaktissa, jotkut asiakkaat mieluusti antavatkin palautetta. Siitä keskustellaan tiimin sisällä, otetaan kehitysehdotuksia. Tavoitteena kerätä jatkossa esim. palautekyselyn muodossa palautetta. 	Tällä hetkellä projektin jälkeen on retro, ollut vain yhdellä asiakkaalla. Keskusteluissa ja meilikirjeenvaihdoissa tulee joskus palautetta. Ei säännöllistä palautetta. 
How


%TODO: feedback: totally different, customer representatives etc
%Feedback now
...
%
%-
%-If we want to shorten the feedback loop, we have to deliver things faster.
%-
%-Compared to B2C the end user amount is very low
%-
